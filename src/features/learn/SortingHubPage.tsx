...
export default function SortingHubPage() {
  ...
  const sortingAlgos = Object.keys(ALGO_META)
    .filter(k => ALGO_META[k as keyof typeof ALGO_META].type === 'sorting')
    .map(k => ({
      id: k,
      ...
      <div className="max-w-[1200px] mx-auto space-y-12 pb-20">
        <header className="space-y-6">
          ...
          <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent flex items-center gap-4">
            <ArrowDownAZ size={40} className="text-blue-400" />
            Sorting Algorithms
          </h1>
          <p className="text-[var(--color-text-secondary)] text-lg max-w-3xl leading-relaxed">
            Sorting algorithms are instructions that take an array or list as input and arrange the items into a particular order (usually alphabetical or numerical). This is a foundational concept in computer science.
          </p>
        </header>
