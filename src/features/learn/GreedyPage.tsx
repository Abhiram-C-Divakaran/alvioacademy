...
export default function GreedyPage() {
  ...
  const greedyAlgos = Object.keys(ALGO_META)
    .filter(k => ALGO_META[k as keyof typeof ALGO_META].type === 'greedy')
    .map(k => ({
      id: k,
      ...
      <div className="max-w-[1200px] mx-auto space-y-12 pb-20">
        <header className="space-y-6">
          ...
          <p className="text-[var(--color-text-secondary)] text-lg max-w-3xl leading-relaxed">
            Greedy algorithms build up a solution piece by piece, always choosing the next piece that offers the most obvious and immediate benefit (the "greedy" choice), with the hope that this choice leads to a globally optimal solution.
          </p>
        </header>
          ...
          {greedyAlgos.map((algo, i) => (
            ...
            >
              <Link to={`/learn/algorithms/${algo.id}`} className="block group">
                <div className="bg-[var(--color-surface-glass)] p-6 rounded-2xl border border-[var(--color-border-subtle)] hover:border-amber-500/50 transition-all h-full hover:-translate-y-1 shadow-sm flex flex-col">
                  <h3 className="text-xl font-bold text-white mb-2 group-hover:text-amber-400 transition-colors">{algo.name}</h3>
