...
export default function SearchingHubPage() {
  ...
  const searchingAlgos = Object.keys(ALGO_META)
    .filter(k => ALGO_META[k as keyof typeof ALGO_META].type === 'searching')
    .map(k => ({
      id: k,
      ...
      <div className="max-w-[1200px] mx-auto space-y-12 pb-20">
        <header className="space-y-6">
          ...
          <p className="text-[var(--color-text-secondary)] text-lg max-w-3xl leading-relaxed">
            Searching algorithms are designed to retrieve information stored within some data structure, whether sorted or unsorted.
          </p>
        </header>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {searchingAlgos.map((algo, i) => (
            <motion.div 
              key={algo.id}
            ...
            >
              <Link to={`/learn/algorithms/${algo.id}`} className="block group">
                <div className="bg-[var(--color-surface-glass)] p-6 rounded-2xl border border-[var(--color-border-subtle)] hover:border-indigo-500/50 transition-all h-full hover:-translate-y-1 shadow-sm flex flex-col">
                  <h3 className="text-xl font-bold text-white mb-2 group-hover:text-indigo-400 transition-colors">{algo.name}</h3>
