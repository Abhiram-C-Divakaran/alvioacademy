...
export default function DivideConquerPage() {
  ...
  // We group both divide-conquer search/sort algos and recursion (hanoi) here
  const dcAlgos = Object.keys(ALGO_META)
    .filter(k => 
      ALGO_META[k as keyof typeof ALGO_META].type === 'recursion' || 
      k === 'binary-search' || 
      k === 'merge-sort' || 
      k === 'quick-sort'
    )
      ...
      <div className="max-w-[1200px] mx-auto space-y-12 pb-20">
        <header className="space-y-6">
          ...
          <p className="text-[var(--color-text-secondary)] text-lg max-w-3xl leading-relaxed">
            Recursion solves a problem by calling itself on smaller instances. Divide and Conquer is an algorithmic paradigm based on multi-branched recursion: it divides the problem into subproblems, conquers them recursively, and combines their solutions.
          </p>
        </header>
